export default function MessageBubble({ message, isSent, avatar, timestamp }) {
  return (
    <div className={`flex gap-4 max-w-[85%] ${isSent ? 'self-end flex-row-reverse' : ''}`}>
      <div className={`w-10 h-10 rounded-full shrink-0 flex items-center justify-center font-bold mt-auto mb-6 ${isSent ? 'bg-echo-bg border border-echo-border' : 'bg-echo-border relative'}`}>
        {avatar}
        {!isSent && (
          <div className="absolute -bottom-1 -right-1 bg-blue-500 rounded-full p-0.5 border-2 border-echo-white">
